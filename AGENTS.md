...
This file is for AI coding agents like **Codex**, **Droid**, **Cursor**, **Windsurf**, **Aider**, **Continue**, and others.

For Claude Code specifically, see `CLAUDE.md` which includes Claude-specific MCP syntax and tools.
...
4. **DEEPSHAFT** - Micro-iteration with lane-based parallel development (no git worktrees)
...
When searching for version info, append the current date:
- "Next.js latest stable version as of January 15, 2026"
- "React best practices as of January 15, 2026"

**NEVER use training data for "latest version" claims. ALWAYS verify.**
...
- Verify accuracy with web search before presenting
...
- Selective triggering reduces tokens 92% while improving accuracy 13.5%
...
- **Auth**: Supabase Auth (PKCE) - Avoid Auth.js (breaking changes as of Jan 2026)
- **Edge Functions**: Deno runtime
- **Deployment**: Vercel - Native Next.js 16 support

### Recommended Tools & Integrations

| Tool | Purpose | Why Default |
|------|---------|-------------|
...
| **Web Search** | Live docs | Verify tech decisions against current info |

**Specky Rule**: If two technologies are equivalent for the user, choose the one with better AI tooling support.
...
- Import directly from packages, avoid barrel files
...
- Chain-of-verification with web search
...
- Tech stack evaluation (with version verification)
- Trade-off analysis
...
### Phase 4: Execution
```
Tasks → Agent Execution → Working Code
```
...
- [ ] Temporal grounding complete (date verified via `date` command)
- [ ] Web search performed for current versions
- [ ] 3-5 clarifying questions answered
...
### Words to Avoid
- **"likely"** - Indicates verification was skipped
- **"should"** - Indicates commitment was avoided

### Anti-Patterns
- ❌ Skip temporal grounding (checking current date)
- ❌ Use stale documentation or training data for versions
- ❌ Execute without user approval
- ❌ Hardcode version numbers without web verification
- ❌ Recommend tech without considering tooling/integration quality
...
**CRITICAL PRINCIPLE**: When two technologies are functionally equivalent for the user, **always choose the one with better AI tooling support**.

### Why This Matters

A well-written Specky spec + good tooling = AI can execute autonomously.
A well-written Specky spec + poor tooling = AI needs human intervention.

### Default Stack (Tooling-Optimized)

| Need | Default Choice | Why |
|------|----------------|-----|
| Database | Supabase | Migration tools, type generation, SQL execution |
| Deployment | Vercel | Native Next.js support, easy config |
| Testing | Playwright | Reliable automation, good AI integration |
| Auth | Supabase Auth | Bundled with database, PKCE by default |
...
If answer is "no preference" → default to tooling-optimized stack.

---

## Closed Learning Events (CLE)

Mistakes made once, learned forever.
...
# Then web search: "[package] latest stable version as of [today's date]"
```

### CLE-002: Tooling-Blind Recommendations (Jan 15, 2026)

**Trigger**: Recommending any backend/database/deployment tech
**Failure**: Could have recommended tech without good AI tooling
**Root Cause**: Didn't weight tooling availability in decisions
**Correct Behavior**: Check tooling/integration quality before recommending. If equivalent options exist, prefer the one with better AI support.
...
npm info next version                   # Verify Next.js version
npm info react version                  # Verify React version
npm info tailwindcss version            # Verify Tailwind version
npm info typescript version             # Verify TypeScript version
```
...
This file (`AGENTS.md`) differs from `CLAUDE.md` in these ways:

| Aspect | CLAUDE.md | AGENTS.md |
|--------|-----------|-----------|
| MCP syntax | Claude-specific (`mcp__*`) | Generic descriptions |
| Tool references | Claude Code tools | Generic "web search", "file tools" |
| Agent patterns | Task tool with subagent_type | Generic agent descriptions |
| Memory protocol | Memory MCP specific | Omitted (agent-specific) |

The core principles, tech stack, and philosophy are identical.
